    ...
    if (config->report_maps_len) {
        ...
        for (uint8_t d = 0; d < dev->devices_len; d++) {
            ...
            if (dev->devices[d].reports == NULL) {
                ESP_LOGE(__func__, "reports malloc(%d) failed", rmap->reports_len * sizeof(hidd_report_item_t));
                free(rmap);
                return ESP_ERR_NO_MEM;
            }
...
static hidd_report_item_t *get_report_by_idx_id_type(esp_bt_hidd_dev_t *dev, size_t index, uint8_t id, uint8_t type)
{
    ...
    if (index >= dev->devices_len) {
        ESP_LOGE(__func__, "index out of range[0-%d]", dev->devices_len - 1);
        return NULL;
    }
...
static bool esp_bt_hidd_dev_connected(void *devp)
{
    ...
    do {
        if (!is_init()) {
            ESP_LOGE(__func__, "HID device profile is uninit");
            ret = false;
            break;
        }
...
static esp_err_t esp_bt_hidd_dev_disconnect(void *devp)
{
    ...
    do {
        if (!is_init()) {
            ESP_LOGE(__func__, "HID device is uninit");
            ret = ESP_FAIL;
            break;
        }
        ...
        if (!dev->connected) {
            osi_mutex_unlock(&s_hidd_param.mutex);
            ESP_LOGW(__func__, "already disconnected");
            return ESP_OK;
        }
    } while(0);
    osi_mutex_unlock(&s_hidd_param.mutex);
...
static esp_err_t esp_bt_hidd_dev_battery_set(void *devp, uint8_t level)
{
    UNUSED(devp);
    UNUSED(level);
    ESP_LOGW(__func__, "Not implement yet!");
    return ESP_OK;
}
...
static esp_err_t esp_bt_hidd_dev_input_set(void *devp, size_t index, size_t id, uint8_t *data, size_t length)
{
    ...
    do {
        ...
        if (!dev->connected) {
            ESP_LOGE(__func__, "HID device not connected!");
            ret = ESP_FAIL;
            break;
        }
        ...
        if (length > p_rpt->value_len) {
            ESP_LOGE(__func__, "Data size over %d!", p_rpt->value_len);
            ret = ESP_FAIL;
            break;
        }
    } while(0);
...
void bt_hidd_cb(esp_hidd_cb_event_t event, esp_hidd_cb_param_t *param)
{
    ...
    if (!is_init()) {
        ESP_LOGE(__func__, "HID device is uninit, event(%d)", event);
        return;
    }
    ...
    case ESP_HIDD_INIT_EVT: {
        if (param->init.status == ESP_HIDD_SUCCESS) {
            ESP_LOGD(__func__, "Setting hid parameters in_qos:%d, out_qos:%d", s_hidd_param.in_qos.token_bucket_size,
            ...
            ESP_LOGE(__func__, "Init hidd failed (%d)!", param->init.status);
    ...
    case ESP_HIDD_DEINIT_EVT: {
        ...
        if (param->deinit.status == ESP_HIDD_SUCCESS) {
            ...
            ESP_LOGE(__func__, "Deinit hidd failed (%d)!", param->deinit.status);
        }
        break;
    }
    case ESP_HIDD_REGISTER_APP_EVT: {
        if (param->register_app.status == ESP_HIDD_SUCCESS) {
            ESP_LOGD(__func__, "Setting hid parameters success!");
            if (param->register_app.in_use) {
                ESP_LOGI(__func__, "Start virtual cable plug!");
            ...
            ESP_LOGE(__func__, "Setting hid parameters failed (%d), now deint!", param->register_app.status);
    ...
    case ESP_HIDD_OPEN_EVT: {
        ...
        if (param->open.status == ESP_HIDD_SUCCESS && param->open.conn_status == ESP_HIDD_CONN_STATE_CONNECTED) {
            ESP_LOGI(__func__, "Connected to %02x:%02x:%02x:%02x:%02x:%02x", param->open.bd_addr[0], param->open.bd_addr[1],
            ...
            ESP_LOGE(__func__, "Connect failed (%d)!", param->open.status);
        }
    ...
    case ESP_HIDD_CLOSE_EVT: {
        ...
        if (param->close.status == ESP_HIDD_SUCCESS && param->close.conn_status == ESP_HIDD_CONN_STATE_DISCONNECTED) {
            ...
            ESP_LOGE(__func__, "Disconnect failed (%d)!", param->close.status);
        }
    ...
    case ESP_HIDD_GET_REPORT_EVT: {
        ...
        p_rpt = get_report_by_id_and_type(s_hidd_param.dev, param->get_report.report_id, param->get_report.report_type,
                                          &map_index);
        if (p_rpt == NULL) {
            ESP_LOGE(__func__, "Can not find report!");
    ...
    case ESP_HIDD_SET_PROTOCOL_EVT: {
        if (param->set_protocol.protocol_mode != ESP_HIDD_UNSUPPORTED_MODE) {
            ...
            ESP_LOGE(__func__, "Unsupported protocol mode!");
            break;
        }
        break;
    }
...
esp_err_t esp_bt_hidd_dev_init(esp_hidd_dev_t *dev_p, const esp_hid_device_config_t *config, esp_event_handler_t callback)
{
    ...
    if (s_hidd_param.mutex == NULL) {
        ESP_LOGE(__func__, "HID device mutex could not be allocated");
        return ESP_ERR_NO_MEM;
    }
