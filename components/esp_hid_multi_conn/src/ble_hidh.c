...
static esp_gatt_status_t read_char(esp_gatt_if_t gattc_if, uint16_t conn_id, uint16_t handle, esp_gatt_auth_req_t auth_req, uint8_t **out, uint16_t *out_len)
{
    ...
    if (esp_ble_gattc_read_char(gattc_if, conn_id, handle, auth_req) != ESP_OK) {
        ESP_LOGE(__func__, "read_char failed");
...
static esp_gatt_status_t read_descr(esp_gatt_if_t gattc_if, uint16_t conn_id, uint16_t handle, esp_gatt_auth_req_t auth_req, uint8_t **out, uint16_t *out_len)
{
    ...
    if (esp_ble_gattc_read_char_descr(gattc_if, conn_id, handle, auth_req) != ESP_OK) {
        ESP_LOGE(__func__, "esp_ble_gattc_read_char failed");
        return ESP_GATT_ERROR;
    }
    WAIT_CB();
    if (s_read_status == ESP_GATT_OK) {
...
static void read_device_services(esp_gatt_if_t gattc_if, esp_hidh_dev_t *dev)
{
    ...
    if (esp_ble_gattc_get_service(gattc_if, dev->ble.conn_id, NULL, service_result, &dcount, 0) == ESP_OK) {
        ESP_LOGD(__func__, "Found %u HID Services", dev->config.report_maps_len);
        ...
        for (uint16_t s = 0; s < dcount; s++) {
            ...
            ESP_LOGV(__func__, "SRV(%d) %s start_handle %d, end_handle %d, uuid: 0x%04x", s, service_result[s].is_primary ? " PRIMARY" : "", service_result[s].start_handle, service_result[s].end_handle, suuid);
            ...
            if (esp_ble_gattc_get_all_char(gattc_if, dev->ble.conn_id, service_result[s].start_handle, service_result[s].end_handle, char_result, &ccount, 0) == ESP_OK) {
                for (uint16_t c = 0; c < ccount; c++) {
                    ...
                    ESP_LOGV(__func__, "  CHAR:(%d), handle: %d, perm: 0x%02x, uuid: 0x%04x", c + 1, chandle, char_result[c].properties, cuuid);
                    ...
                    if (suuid == 0x1800) {
                        ...
                        if (cuuid == ESP_GATT_UUID_HID_REPORT_MAP) {
                            ...
                            if (report == NULL) {
                                ESP_LOGE(__func__, "malloc esp_hidh_dev_report_t failed");
                                return;
                            }
                            report->next = NULL;
                    ...
                    if (esp_ble_gattc_get_all_descr(gattc_if, dev->ble.conn_id, char_result[c].char_handle, descr_result, &dcount, 0) == ESP_OK) {
                        for (uint16_t d = 0; d < dcount; d++) {
                            ...
                            ESP_LOGV(__func__, "    DESCR:(%d), handle: %d, uuid: 0x%04x", d + 1, dhandle, duuid);
...
void esp_hidh_gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_REG_EVT:
        if (param->reg.status == ESP_GATT_OK) {
            ...
            ESP_LOGE(__func__, "Reg app failed, app_id %04x, status 0x%x", param->reg.app_id,  param->reg.status);
            return;
        }
        SEND_CB();
        break;
    ...
    case ESP_GATTC_OPEN_EVT:
        ESP_LOGV(__func__, "OPEN bda " ESP_BD_ADDR_STR ", conn_id %d, status 0x%x, mtu %d", ESP_BD_ADDR_HEX(p_data->open.remote_bda), p_data->open.conn_id, p_data->open.status, p_data->open.mtu);
        dev = esp_hidh_dev_get_by_bda(p_data->open.remote_bda);
        if (!dev) {
            ESP_LOGE(__func__, "OPEN received for unknown device");
            break;
        }
        if (p_data->open.status != 0) {
            //error
            ESP_LOGE(__func__, "OPEN failed: 0x%x", p_data->open.status);
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        if (!dev) {
            ESP_LOGE(__func__, "SEARCH_CMPL received for unknown device");
            break;
        }
        if (dev->status == ESP_GATT_NOT_FOUND) {
            //service not found
            ESP_LOGE(__func__, "SEARCH_CMPL HID Service was not found on the device");
    ...
    case ESP_GATTC_READ_DESCR_EVT: {
        ...
        if (!dev) {
            ESP_LOGE(__func__, "READ received for unknown device");
            break;
        }
    ...
    case ESP_GATTC_WRITE_DESCR_EVT: {
        ...
        if (!dev) {
            ESP_LOGE(__func__, "WRITE_DESCR received for unknown device");
            break;
        }
    ...
    case ESP_GATTC_WRITE_CHAR_EVT: {
        dev = esp_hidh_dev_get_by_conn_id(p_data->write.conn_id);
        if (!dev) {
            ESP_LOGE(__func__, "WRITE_CHAR received for unknown device");
            break;
        }
        dev->status = p_data->write.status;
        if (p_data->write.status) {
            ESP_LOGE(__func__, "WRITE_CHAR: conn_id %d, handle %d, status 0x%x", p_data->write.conn_id, p_data->write.handle, p_data->write.status);
        }
        break;
    }
    ...
    case ESP_GATTC_DISCONNECT_EVT: {
        ESP_LOGV(__func__, "DISCONNECT: bda " ESP_BD_ADDR_STR ", conn_id %u, reason 0x%x", ESP_BD_ADDR_HEX(p_data->disconnect.remote_bda), p_data->disconnect.conn_id, p_data->disconnect.reason);
        break;
    }
    ...
    case ESP_GATTC_NOTIFY_EVT: {
        ...
        if (!dev) {
            ESP_LOGE(__func__, "NOTIFY received for unknown device");
            break;
        }
    ...
    case ESP_GATTC_CLOSE_EVT: {
        ESP_LOGV(__func__, "CLOSE bda " ESP_BD_ADDR_STR ", conn_id %d, status 0x%x, reason 0x%x", ESP_BD_ADDR_HEX(p_data->close.remote_bda), p_data->close.conn_id, p_data->close.status, p_data->close.reason);
        dev = esp_hidh_dev_get_by_bda(p_data->open.remote_bda);
        if (!dev) {
            ESP_LOGE(__func__, "CLOSE received for unknown device");
            break;
        }
        if (!dev->connected) {
    ...
    default:
        ESP_LOGV(__func__, "GATTC EVENT %s", gattc_evt_str(event));
        break;
    }
}
...
static esp_err_t esp_ble_hidh_dev_report_write(esp_hidh_dev_t *dev, size_t map_index, size_t report_id, int report_type, uint8_t *value, size_t value_len)
{
    ...
    if (!report) {
        ESP_LOGE(__func__, "%s report %d not found", esp_hid_report_type_str(report_type), report_id);
        return ESP_FAIL;
    }
    if (value_len > report->value_len) {
        ESP_LOGE(__func__, "%s report %d takes maximum %d bytes. you have provided %d", esp_hid_report_type_str(report_type), report_id, report->value_len, value_len);
        return ESP_FAIL;
    }
...
static esp_err_t esp_ble_hidh_dev_report_read(esp_hidh_dev_t *dev, size_t map_index, size_t report_id, int report_type, size_t max_length, uint8_t *value, size_t *value_len)
{
    ...
    ESP_LOGE(__func__, "%s report %d read failed: 0x%x", esp_hid_report_type_str(report_type), report_id, s);
    return ESP_FAIL;
}
...
esp_err_t esp_ble_hidh_init(const esp_hidh_config_t *config)
{
    ...
    if (s_ble_hidh_cb_semaphore != NULL) {
        ESP_LOGE(__func__, "Already initialised");
        return ESP_FAIL;
    }
    ...
    do {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(__func__, "%s esp_event_loop_create failed!", __func__);
            break;
        }

        ret = esp_ble_gattc_app_register(0);
        if (ret != ESP_OK) {
            ESP_LOGE(__func__, "esp_ble_gattc_app_register failed!");
            break;
        }
        WAIT_CB();
...
{
    if (s_ble_hidh_cb_semaphore == NULL) {
        ESP_LOGE(__func__, "Already deinitialised");
        return ESP_FAIL;
    }

    esp_err_t err = esp_ble_gattc_app_unregister(hid_gattc_if);
    if (err != ESP_OK) {
        ESP_LOGE(__func__, "App Unregister Failed");
        return err;
    }
...
esp_hidh_dev_t *esp_ble_hidh_dev_open(esp_bd_addr_t bda, esp_ble_addr_type_t address_type)
{
    ...
    if (dev == NULL) {
        ESP_LOGE(__func__, "malloc esp_hidh_dev_t failed");
        return NULL;
    }
    ...
    if (ret) {
        esp_hidh_dev_free_inner(dev);
        ESP_LOGE(__func__, "esp_ble_gattc_open failed: %d", ret);
        return NULL;
    }
    WAIT_CB();
    if (dev->ble.conn_id < 0) {
        ret = dev->status;
        ESP_LOGE(__func__, "dev open failed! status: 0x%x", dev->status);
        esp_hidh_dev_free_inner(dev);
        return NULL;
    }
