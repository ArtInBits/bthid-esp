...
esp_err_t esp_hidh_init(const esp_hidh_config_t *config)
{
    ...
    if (s_esp_hidh_devices_semaphore != NULL) {
        ESP_LOGE(__func__, "Already initialized");
        return err;
    }
...
esp_err_t esp_hidh_deinit(void)
{
    esp_err_t err = ESP_FAIL;
    if (s_esp_hidh_devices_semaphore == NULL) {
        ESP_LOGE(__func__, "Already uninitialized");
        return err;
    }

    if (!TAILQ_EMPTY(&s_esp_hidh_devices)) {
        ESP_LOGE(__func__, "Please disconnect all devices first!");
        return err;
    }
...
esp_hidh_dev_t *esp_hidh_dev_open(esp_bd_addr_t bda, esp_hid_transport_t transport, uint8_t remote_addr_type)
{
    if (esp_hidh_dev_get_by_bda(bda) != NULL) {
        ESP_LOGE(__func__, "Already Connected");
        return NULL;
    }
...
/**
 * If no Report ID item __func__s are present in the Report descriptor, it
                                                                       ...
                                                                       size_t len, const uint8_t *data, bool *has_report_id)
{
    ...
    *has_report_id = false;
    ...
    if (r == NULL) {
        if (*data == 0) {
            ESP_LOGE(__func__, "data not include report id!");
            *has_report_id = false;
            return NULL;
        }
        r = dev->reports;
...
esp_hidh_dev_t *esp_hidh_dev_malloc()
{
    ...
    if (dev->semaphore == NULL) {
        ESP_LOGE(__func__, "malloc semaphore failed");
    ...
    if (dev->mutex == NULL) {
        ESP_LOGE(__func__, "malloc mutex failed");
        esp_hidh_dev_resources_free(dev);
        return NULL;
    }
...
esp_err_t esp_hidh_dev_free_inner(esp_hidh_dev_t *dev)
{
    ...
    unlock_devices();
    if (ret != ESP_OK) {
        ESP_LOGW(__func__, "device not found");
    } else {
        ESP_LOGD(__func__, "device removed");
    }
    return ret;
}
