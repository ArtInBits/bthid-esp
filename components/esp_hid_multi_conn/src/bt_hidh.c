...
static void transaction_timeout_handler(void *arg)
{
    ...
    if (dev != NULL && esp_hidh_dev_exists(dev)) {
        ESP_LOGW(__func__, "transaction timeout!");
...
static inline void set_trans(esp_hidh_dev_t *dev, esp_hid_trans_type_t trans_type)
{
    ...
    if (dev->trans_timer == NULL) {
        ...
        if (esp_timer_create(&config, &dev->trans_timer) != ESP_OK) {
            ESP_LOGE(__func__, "create trans timer failed! trans:%s", get_trans_type_str(trans_type));
            return;
        }
    }
    if (!esp_timer_is_active(dev->trans_timer) && esp_timer_start_once(dev->trans_timer, TRANS_TO) != ESP_OK) {
        ESP_LOGE(__func__, "set trans timer failed! trans:%s", get_trans_type_str(trans_type));
    }
}
...
static void esp_hh_cb(esp_hidh_cb_event_t event, esp_hidh_cb_param_t *param)
{
    ...
    case ESP_HIDH_INIT_EVT: {
        ...
        if (param->init.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "ENABLE ERROR: %s", s_esp_hh_status_names[param->init.status]);
            free_local_param();
        }
        break;
    }
    case ESP_HIDH_DEINIT_EVT: {
        ...
        if (param->deinit.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "DISABLE ERROR: %s", s_esp_hh_status_names[param->deinit.status]);
        } else {
            free_local_param();
        }
        break;
    ...
    case ESP_HIDH_OPEN_EVT: {
        ...
        do {
            ...
            if (dev == NULL) {
                if (param->open.is_orig) {
                    ESP_LOGE(__func__, "OPEN ERROR: Device Not Found");
                    param->open.status = ESP_HIDH_NO_CONNECTION;
                    break;
                } else {
                    ESP_LOGD(__func__, "incoming device connect");
                    if ((dev = hidh_dev_ctor(param->open.bd_addr)) == NULL) {
                        ESP_LOGE(__func__, "%s create device failed!", __func__);
            ...
            if (conn_item == NULL) {
                ESP_LOGE(__func__, "conn_item malloc failed!");
                param->open.status = ESP_HIDH_ERR_NO_RES;
                break;
            }
            conn_item->dev = dev;
        ...
        if (param->open.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "OPEN ERROR: %s", s_esp_hh_status_names[param->open.status]);
    ...
    case ESP_HIDH_GET_DSCP_EVT: {
        do {
            ESP_LOGV(__func__, "DESCRIPTOR: PID: 0x%04x, VID: 0x%04x, VERSION: 0x%04x, REPORT_LEN: %u",
            ...
            if ((conn_item = (conn_item_t *)fixed_queue_dequeue(hidh_local_param.connection_queue,
                ...
                ESP_LOGE(__func__, "No pending connect device!");
            ...
            if (!esp_hidh_dev_exists(dev)) {
                ESP_LOGE(__func__, "Device Not Found");
                dev = NULL;
                param->dscp.status = ESP_HIDH_NO_CONNECTION;
                break;
            }
            ...
            if (!dev->connected) {
                esp_hidh_dev_unlock(dev);
                ESP_LOGE(__func__, "Connection has been released!");
                param->dscp.status = ESP_HIDH_NO_CONNECTION;
                break;
            }
            ...
            if (param->dscp.status != ESP_HIDH_OK) {
                esp_hidh_dev_unlock(dev);
                ESP_LOGE(__func__, "GET_DSCP ERROR: %s", s_esp_hh_status_names[param->dscp.status]);
                break;
            }
            ...
            if (dev->config.report_maps == NULL) {
                esp_hidh_dev_unlock(dev);
                ESP_LOGE(__func__, "malloc report maps failed");
            ...
            if (dev->config.report_maps[0].data == NULL) {
                ESP_LOGE(__func__, "Malloc Report Map Failed");
                param->dscp.status = ESP_HIDH_ERR_NO_RES;
            } else {
                ...
                if (dev->config.report_maps[0].len && dev->config.report_maps[0].data) {
                    ...
                    if (map) {
                        ...
                        for (uint8_t i = 0; i < map->reports_len; i++) {
                            ...
                            if (report == NULL) {
                                ESP_LOGE(__func__, "Malloc Report Failed");
                                param->dscp.status = ESP_HIDH_ERR_NO_RES;
                                break;
                            }
                            report->map_index = 0;
                        ...
                        ESP_LOGE(__func__, "Parse Report Map Failed");
    ...
    case ESP_HIDH_ADD_DEV_EVT: {
        ESP_LOGV(__func__, "ADD_DEV: BDA: " ESP_BD_ADDR_STR ", handle: %d, status: %s",
        ...
        if (param->add_dev.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "ADD_DEV ERROR: %s", s_esp_hh_status_names[param->add_dev.status]);
    ...
    case ESP_HIDH_CLOSE_EVT: {
        ...
        ESP_LOGV(__func__, "CLOSE: handle: %d, status: %s", param->close.handle, s_esp_hh_status_names[param->close.status]);
        do {
        ...
        if (param->close.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "CLOSE ERROR: %s", s_esp_hh_status_names[param->close.status]);
        }
        p.close.dev = dev;
    ...
    case ESP_HIDH_SET_RPT_EVT: {
        if (param->set_rpt.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "SET_RPT ERROR: handle: %d, status: %s", param->set_rpt.handle,
                     s_esp_hh_status_names[param->set_rpt.status]);
        }
        dev = esp_hidh_dev_get_by_handle(param->set_rpt.handle);
        if (dev == NULL) {
            ESP_LOGE(__func__, "SET_RPT ERROR: Device Not Found");
            break;
        }
        esp_hidh_dev_lock(dev);
    ...
    case ESP_HIDH_GET_RPT_EVT: {
        if (param->get_rpt.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "GET_RPT ERROR: handle: %d, status: %s", param->get_rpt.handle,
        ...
        if (dev == NULL) {
            ESP_LOGE(__func__, "GET_RPT ERROR: Device Not Found");
            break;
        }
        esp_hidh_dev_lock(dev);
        ...
        if ((p_param = (esp_hidh_event_data_t *)malloc(event_data_size)) != NULL) {
            ...
            esp_hidh_dev_unlock(dev);
            ESP_LOGE(__func__, "GET_RPT ERROR: malloc event data failed!");
        }
        reset_trans(dev);
        break;
    }
    case ESP_HIDH_GET_IDLE_EVT:{
        if (param->get_idle.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "GET_IDLE ERROR: handle: %d, status: %s", param->get_idle.handle,
        ...
        if (dev == NULL) {
            ESP_LOGE(__func__, "GET_IDLE ERROR: Device Not Found");
            break;
        }
        esp_hidh_dev_lock(dev);
        ...
        if ((p_param = (esp_hidh_event_data_t *)malloc(event_data_size)) != NULL) {
            ...
            esp_hidh_dev_unlock(dev);
            ESP_LOGE(__func__, "GET_IDLE ERROR: malloc event data failed!");
        }
        reset_trans(dev);
        break;
    }
    case ESP_HIDH_SET_IDLE_EVT: {
        if (param->set_idle.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "SET_IDLE ERROR: handle: %d, status: %s", param->set_idle.handle,
                     s_esp_hh_status_names[param->set_idle.status]);
        }
        dev = esp_hidh_dev_get_by_handle(param->set_idle.handle);
        if (dev == NULL) {
            ESP_LOGE(__func__, "SET_IDLE ERROR: Device Not Found");
            break;
        }
        esp_hidh_dev_lock(dev);
    ...
    case ESP_HIDH_GET_PROTO_EVT: {
        if (param->get_proto.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "GET_PROTO ERROR: handle: %d, status: %s", param->get_proto.handle,
        ...
        if (dev == NULL) {
            ESP_LOGE(__func__, "GET_PROTO ERROR: Device Not Found");
            break;
        }
        esp_hidh_dev_lock(dev);
        ...
        if ((p_param = (esp_hidh_event_data_t *)malloc(event_data_size)) != NULL) {
            ...
            esp_hidh_dev_unlock(dev);
            ESP_LOGE(__func__, "GET_PROTO ERROR: malloc event data failed!");
        }
        reset_trans(dev);
        break;
    }
    case ESP_HIDH_SET_PROTO_EVT: {
        if (param->set_proto.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "SET_PROTO ERROR: handle: %d, status: %s", param->set_proto.handle,
    ...
    case ESP_HIDH_DATA_IND_EVT: {
        ...
        if (param->data_ind.status != ESP_HIDH_OK) {
            ESP_LOGE(__func__, "DATA_IND ERROR: handle: %d, status: %s", param->data_ind.handle,
                     s_esp_hh_status_names[param->data_ind.status]);
        }
        dev = esp_hidh_dev_get_by_handle(param->data_ind.handle);
        if (dev == NULL) {
            ESP_LOGE(__func__, "Device Not Found: handle %u", param->data_ind.handle);
            break;
        }
        ...
        if (param->data_ind.len > 0 && param->data_ind.data != NULL) {
            ...
            if (param->data_ind.proto_mode == ESP_HID_PROTOCOL_MODE_BOOT) {
                ...
                if (param->data_ind.len == 9 && *(param->data_ind.data) == 1) {
                    ...
                    esp_hidh_dev_unlock(dev);
                    ESP_LOGE(__func__, "Invalid Boot Report format, rpt_len:%d, rpt_id:%d!", param->data_ind.len,
                             *(param->data_ind.data));
                    break;
                }
            } else {
                ...
                if (report == NULL) {
                    esp_hidh_dev_unlock(dev);
                    ESP_LOGE(__func__, "Not find report handle: %d mode: %s", param->data_ind.handle,
            ...
            if ((p_param = (esp_hidh_event_data_t *)malloc(event_data_size)) == NULL) {
                esp_hidh_dev_unlock(dev);
                ESP_LOGE(__func__, "DATA_IND ERROR: malloc event data failed!");
                break;
            }
            memset(p_param, 0, event_data_size);
    ...
    case ESP_HIDH_DATA_EVT:
        break;
    default:
        ESP_LOGV(__func__, "BTA_HH EVENT: %s", s_esp_hh_evt_names[event]);
        break;
    }
...
static esp_err_t esp_bt_hidh_dev_close(esp_hidh_dev_t *dev)
{
    ...
    do {
        ...
        if (!dev->connected) {
            ESP_LOGW(__func__, "%s hdl:0x%02x not connected", __func__, dev->bt.handle);
                                              ...
                                              int report_type, uint8_t *data, size_t len)
{
    ...
    do {
        ...
        if (!report) {
            ESP_LOGE(__func__, "mode:%s report:%s id:%d not found", get_protocol_mode(dev->protocol_mode),
                     esp_hid_report_type_str(report_type), report_id);
            ret = ESP_FAIL;
            break;
        }
        if (len > report->value_len) {
            ESP_LOGE(__func__, "%s report %d takes maximum %d bytes. you have provided %d",
        ...
        if (report_id) {
            if ((p_data = malloc(len + 1)) == NULL) {
                ESP_LOGE(__func__, "%s malloc failed!", __func__);
                ret = ESP_FAIL;
                break;
            }
            *p_data = report_id;
                                            ...
                                            int report_type, uint8_t *data, size_t len)
{
    ...
    do {
        if (!is_trans_done(dev)) {
            ESP_LOGE(__func__, "Pending previous tansaction %s done, try later!", get_trans_type_str(dev->trans_type));
            ret = ESP_FAIL;
            break;
        }
...
esp_err_t esp_bt_hidh_init(const esp_hidh_config_t *config)
{
    esp_err_t ret = ESP_OK;
    if (config == NULL) {
        ESP_LOGE(__func__, "Config is NULL");
    ...
    do {
        if ((hidh_local_param.connection_queue = fixed_queue_new(QUEUE_SIZE_MAX)) == NULL) {
            ESP_LOGE(__func__, "connection_queue create failed!");
            ret = ESP_FAIL;
            break;
        }
        ret = esp_event_loop_create(&event_task_args, &hidh_local_param.event_loop_handle);
        if (ret != ESP_OK) {
            ESP_LOGE(__func__, "esp_event_loop_create failed!");
            ret = ESP_FAIL;
            break;
        }
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(__func__, "event_loop register failed!");
            ret = ESP_FAIL;
            break;
        }
...
esp_hidh_dev_t *esp_bt_hidh_dev_open(esp_bd_addr_t bda)
{
    ...
    if (dev == NULL) {
        ...
        ESP_LOGW(__func__, "device has opened, connected: %d", dev->connected);
    }
