...
esp_err_t esp_hidd_dev_raw_input_set(esp_hidd_dev_t *dev, size_t map_index, size_t report_id, uint8_t *data, size_t length)
{
    if (dev == NULL) {
        return ESP_FAIL;
    }
    return dev->raw_input_set(dev->dev, map_index, report_id, data, length);
}
...
esp_err_t esp_hidd_dev_next_client(esp_hidd_dev_t *dev)
{
    ...
    return dev->next_client(dev->dev);
}
...
esp_err_t esp_hidd_dev_select_client(esp_hidd_dev_t *dev, int idx)
{
    ...
    return dev->select_client(dev->dev, idx);
}
