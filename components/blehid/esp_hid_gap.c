...
{
    ESP_LOGI(__func__, "entering: event = %d", event);
    ...
    case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT:
        ESP_LOGV(__func__, "BLE GAP ADV_DATA_SET_COMPLETE");
        break;

    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
        ESP_LOGV(__func__, "BLE GAP ADV_START_COMPLETE");
        break;
    ...
    case ESP_GAP_BLE_AUTH_CMPL_EVT:
        if (!param->ble_security.auth_cmpl.success) {
            ESP_LOGE(__func__, "BLE GAP AUTH ERROR: 0x%x", param->ble_security.auth_cmpl.fail_reason);
        } else {
            ESP_LOGI(__func__, "BLE GAP AUTH SUCCESS: client address: %02x:%02x:%02x:%02x:%02x:%02x",
            ...
            //add this central to the list of served clients
        }
        //resume advertising
    ...
    case ESP_GAP_BLE_KEY_EVT: //shows the ble key info share with peer device to the user.
        ESP_LOGI(__func__, "BLE GAP KEY type = %s", esp_ble_key_type_str(param->ble_security.ble_key.key_type));
        break;

    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT: // ESP_IO_CAP_OUT
        // The app will receive this evt when the IO has Output capability and the peer device IO has Input capability.
        // Show the passkey number to the user to input it in the peer device.
        ESP_LOGI(__func__, "BLE GAP PASSKEY_NOTIF passkey:%"PRIu32, param->ble_security.key_notif.passkey);
        break;

    case ESP_GAP_BLE_NC_REQ_EVT: // ESP_IO_CAP_IO
        // The app will receive this event when the IO has DisplayYesNO capability and the peer device IO also has DisplayYesNo capability.
        // show the passkey number to the user to confirm it with the number displayed by peer device.
        ESP_LOGI(__func__, "BLE GAP NC_REQ passkey:%"PRIu32, param->ble_security.key_notif.passkey);
    ...
    case ESP_GAP_BLE_PASSKEY_REQ_EVT: // ESP_IO_CAP_IN
        ...
        ESP_LOGI(__func__, "BLE GAP PASSKEY_REQ");
        //esp_ble_passkey_reply(param->ble_security.ble_req.bd_addr, true, 1234);
        break;

    case ESP_GAP_BLE_SEC_REQ_EVT:
        ESP_LOGI(__func__, "BLE GAP SEC_REQ");
    ...
    default:
        ESP_LOGV(__func__, "BLE GAP EVENT %s", ble_gap_evt_str(event));
        break;
    }
}
...
static esp_err_t init_ble_gap(void)
{
    ...
    if ((ret = esp_ble_gap_register_callback(ble_gap_event_handler)) != ESP_OK) {
        ESP_LOGE(__func__, "esp_ble_gap_register_callback failed: %d", ret);
        return ret;
    }
    return ret;
}
...
static esp_err_t init_low_level(uint8_t mode)
{
...
#endif
    {
        ...
        if (ret) {
            ESP_LOGE(__func__, "esp_bt_controller_mem_release failed: %d", ret);
            return ret;
        }
    }
    ret = esp_bt_controller_init(&bt_cfg);
    if (ret) {
        ESP_LOGE(__func__, "esp_bt_controller_init failed: %d", ret);
        return ret;
    }

    ret = esp_bt_controller_enable(mode);
    if (ret) {
        ESP_LOGE(__func__, "esp_bt_controller_enable failed: %d", ret);
        return ret;
    }

    ret = esp_bluedroid_init();
    if (ret) {
        ESP_LOGE(__func__, "esp_bluedroid_init failed: %d", ret);
        return ret;
    }

    ret = esp_bluedroid_enable();
    if (ret) {
        ESP_LOGE(__func__, "esp_bluedroid_enable failed: %d", ret);
        return ret;
    }
...
esp_err_t esp_hid_gap_init(uint8_t mode)
{
    ...
    if (!mode || mode > ESP_BT_MODE_BTDM) {
        ESP_LOGE(__func__, "Invalid mode given!");
        return ESP_FAIL;
    }
